// legalTranslations.js
// Übersetzungen für alle Legal Pages (Impressum, Datenschutz, AGB, etc.)

export const LEGAL_TRANSLATIONS = {
  en: {
    // Navigation
    backToQuiz: "Back to Quiz",
    
    // Page Names
    impressum: "Legal Notice",
    datenschutz: "Privacy Policy",
    agb: "Terms of Service", 
    kontakt: "Contact",
    
    // Impressum
    impressumTitle: "Legal Notice",
    tmgHeading: "Information according to § 5 TMG",
    contactHeading: "Contact",
    taxIdHeading: "Tax ID", 
    responsibleHeading: "Responsible for content according to § 55 para. 2 RStV",
    contentLiabilityHeading: "Liability for content",
    linksLiabilityHeading: "Liability for links",
    copyrightHeading: "Copyright",
    
    tmgText: "EVAL(IA) - Alberto Sejas Knapp\nEinsteinstrasse 59\nDE71229, Leonberg\nGermany",
    contactText: "Phone: +49 170 967 0950\nEmail: kontakt@evaliaquiz.com",
    taxIdText: "VAT identification number according to § 27 a VAT law:\nTax No. 97 099 20751",
    responsibleText: "Alberto Sejas Knapp\nEinsteinstrasse 59\nDE71229, Leonberg",
    contentLiabilityText: "As a service provider, we are responsible for our own content on these pages according to general laws pursuant to § 7 para. 1 TMG. However, according to §§ 8 to 10 TMG, we are not obligated as a service provider to monitor transmitted or stored third-party information or to investigate circumstances that indicate illegal activity.",
    linksLiabilityText: "Our offer contains links to external websites of third parties, over whose content we have no influence. Therefore, we cannot assume any liability for this external content. The respective provider or operator of the pages is always responsible for the content of the linked pages.",
    copyrightText: "The content and works created by the site operators on these pages are subject to German copyright law. Duplication, processing, distribution and any form of commercialization of such material beyond the scope of the copyright law requires the written consent of its respective author or creator.",
    
    // Privacy Policy
    datenschutzTitle: "Privacy Policy",
    dataProtectionHeading: "Data Protection",
    dataCollectionHeading: "Data Collection",
    cookiesHeading: "Cookies",
    analyticsHeading: "Analytics",
    rightsHeading: "Your Rights",
    contactDataHeading: "Contact for Data Protection",
    
    dataProtectionText: "We take the protection of your personal data very seriously. We treat your personal data confidentially and in accordance with the statutory data protection regulations and this privacy policy.",
    dataCollectionText: "We collect and use personal data only insofar as this is necessary for the provision of a functional website as well as our content and services.",
    cookiesText: "This website uses cookies to improve user experience. By using our website, you consent to the use of cookies in accordance with this privacy policy.",
    analyticsText: "We use web analytics services to analyze the use of our website and to continuously improve our offering.",
    rightsText: "You have the right to information, correction, deletion, restriction, data portability, revocation and objection regarding your personal data.",
    contactDataText: "For questions regarding data protection, please contact us at: [YOUR EMAIL]",
    
    // Terms of Service
    agbTitle: "Terms of Service",
    scopeHeading: "Scope of Application",
    servicesHeading: "Services",
    userObligationsHeading: "User Obligations", 
    liabilityHeading: "Liability",
    modificationHeading: "Modifications",
    finalProvisionsHeading: "Final Provisions",
    
    scopeText: "These terms of service apply to all services provided by EVALIA through this website.",
    servicesText: "EVALIA provides an AI-powered knowledge quiz platform. The service is provided as-is without warranties.",
    userObligationsText: "Users must provide accurate information and use the service in accordance with applicable laws and these terms.",
    liabilityText: "Liability is limited to the extent permitted by applicable law. EVALIA is not liable for indirect damages.",
    modificationText: "We reserve the right to modify these terms at any time. Users will be notified of significant changes.",
    finalProvisionsText: "German law applies. Place of jurisdiction is Leonberg, Germany.",
    
    // Contact
    kontaktTitle: "Contact",
    contactFormHeading: "Contact Form",
    nameLabel: "Name",
    emailLabel: "Email",
    messageLabel: "Message",
    sendButton: "Send Message",
    contactInfoHeading: "Contact Information",
    addressHeading: "Address",
    phoneHeading: "Phone",
    emailHeading: "Email"
  },
  
  de: {
    // Navigation
    backToQuiz: "Zurück zum Quiz",
    
    // Page Names  
    impressum: "Impressum",
    datenschutz: "Datenschutz",
    agb: "AGB",
    kontakt: "Kontakt",
    
    // Impressum
    impressumTitle: "Impressum",
    tmgHeading: "Angaben gemäß § 5 TMG",
    contactHeading: "Kontakt", 
    taxIdHeading: "Umsatzsteuer-ID",
    responsibleHeading: "Verantwortlich für den Inhalt nach § 55 Abs. 2 RStV",
    contentLiabilityHeading: "Haftung für Inhalte",
    linksLiabilityHeading: "Haftung für Links",
    copyrightHeading: "Urheberrecht",
    
    tmgText: "EVAL(IA) - Alberto Sejas Knapp\nEinsteinstrasse 59\nDE71229, Leonberg\nDeutschland",
    contactText: "Telefon: +49 170 967 0950\nE-Mail: kontakt@evaliaquiz.com",
    taxIdText: "Umsatzsteuer-Identifikationsnummer gemäß § 27 a Umsatzsteuergesetz:\nSt-Nr 97 099 20751",
    responsibleText: "Alberto Sejas Knapp\nEinsteinstrasse 59\nDE71229, Leonberg",
    contentLiabilityText: "Als Diensteanbieter sind wir gemäß § 7 Abs.1 TMG für eigene Inhalte auf diesen Seiten nach den allgemeinen Gesetzen verantwortlich. Nach §§ 8 bis 10 TMG sind wir als Diensteanbieter jedoch nicht unter der Verpflichtung, übermittelte oder gespeicherte fremde Informationen zu überwachen oder nach Umständen zu forschen, die auf eine rechtswidrige Tätigkeit hinweisen.",
    linksLiabilityText: "Unser Angebot enthält Links zu externen Websites Dritter, auf deren Inhalte wir keinen Einfluss haben. Deshalb können wir für diese fremden Inhalte auch keine Gewähr übernehmen. Für die Inhalte der verlinkten Seiten ist stets der jeweilige Anbieter oder Betreiber der Seiten verantwortlich.",
    copyrightText: "Die durch die Seitenbetreiber erstellten Inhalte und Werke auf diesen Seiten unterliegen dem deutschen Urheberrecht. Die Vervielfältigung, Bearbeitung, Verbreitung und jede Art der Verwertung außerhalb der Grenzen des Urheberrechtes bedürfen der schriftlichen Zustimmung des jeweiligen Autors bzw. Erstellers.",
    
    // Datenschutz
    datenschutzTitle: "Datenschutzerklärung",
    dataProtectionHeading: "Datenschutz",
    dataCollectionHeading: "Datenerhebung",
    cookiesHeading: "Cookies",
    analyticsHeading: "Analyse-Tools",
    rightsHeading: "Ihre Rechte",
    contactDataHeading: "Kontakt Datenschutz",
    
    dataProtectionText: "Wir nehmen den Schutz Ihrer persönlichen Daten sehr ernst. Wir behandeln Ihre personenbezogenen Daten vertraulich und entsprechend der gesetzlichen Datenschutzvorschriften sowie dieser Datenschutzerklärung.",
    dataCollectionText: "Wir erheben und verwenden personenbezogene Daten nur, soweit dies zur Bereitstellung einer funktionsfähigen Website sowie unserer Inhalte und Leistungen erforderlich ist.",
    cookiesText: "Diese Website verwendet Cookies zur Verbesserung der Nutzererfahrung. Durch die Nutzung unserer Website stimmen Sie der Verwendung von Cookies gemäß dieser Datenschutzerklärung zu.",
    analyticsText: "Wir setzen Web-Analyse-Dienste ein, um die Nutzung unserer Website zu analysieren und unser Angebot stetig zu verbessern.",
    rightsText: "Sie haben das Recht auf Auskunft, Berichtigung, Löschung, Einschränkung, Datenübertragbarkeit, Widerruf und Widerspruch bezüglich Ihrer personenbezogenen Daten.",
    contactDataText: "Bei Fragen zum Datenschutz wenden Sie sich bitte an: [IHRE E-MAIL]",
    
    // AGB
    agbTitle: "Allgemeine Geschäftsbedingungen",
    scopeHeading: "Geltungsbereich",
    servicesHeading: "Leistungen",
    userObligationsHeading: "Pflichten der Nutzer",
    liabilityHeading: "Haftung",
    modificationHeading: "Änderungen",
    finalProvisionsHeading: "Schlussbestimmungen",
    
    scopeText: "Diese Allgemeinen Geschäftsbedingungen gelten für alle über diese Website erbrachten Leistungen von EVALIA.",
    servicesText: "EVALIA stellt eine KI-gestützte Wissensquiz-Plattform zur Verfügung. Die Leistung wird wie besehen ohne Gewährleistungen erbracht.",
    userObligationsText: "Nutzer müssen wahrheitsgemäße Angaben machen und den Service in Übereinstimmung mit geltendem Recht und diesen Bedingungen nutzen.",
    liabilityText: "Die Haftung ist auf das gesetzlich zulässige Maß beschränkt. EVALIA haftet nicht für mittelbare Schäden.",
    modificationText: "Wir behalten uns das Recht vor, diese Bedingungen jederzeit zu ändern. Nutzer werden über wesentliche Änderungen informiert.",
    finalProvisionsText: "Es gilt deutsches Recht. Gerichtsstand ist Leonberg, Deutschland.",
    
    // Kontakt
    kontaktTitle: "Kontakt",
    contactFormHeading: "Kontaktformular",
    nameLabel: "Name",
    emailLabel: "E-Mail",
    messageLabel: "Nachricht",
    sendButton: "Nachricht senden",
    contactInfoHeading: "Kontaktinformationen",
    addressHeading: "Adresse",
    phoneHeading: "Telefon",
    emailHeading: "E-Mail"
  },
  
  fr: {
    // Navigation
    backToQuiz: "Retour au Quiz",
    
    // Page Names
    impressum: "Mentions Légales",
    datenschutz: "Politique de Confidentialité",
    agb: "Conditions d'Utilisation",
    kontakt: "Contact",
    
    // Impressum  
    impressumTitle: "Mentions Légales",
    tmgHeading: "Informations selon § 5 TMG",
    contactHeading: "Contact",
    taxIdHeading: "ID TVA",
    responsibleHeading: "Responsable du contenu selon § 55 para. 2 RStV",
    contentLiabilityHeading: "Responsabilité du contenu",
    linksLiabilityHeading: "Responsabilité des liens",
    copyrightHeading: "Droit d'auteur",
    
    tmgText: "EVAL(IA) - Alberto Sejas Knapp\nEinsteinstrasse 59\nDE71229, Leonberg\nAllemagne",
    contactText: "Téléphone: +49 170 967 0950\nEmail: kontakt@evaliaquiz.com",
    taxIdText: "Numéro d'identification TVA selon § 27 a loi TVA:\nN° fiscal 97 099 20751",
    responsibleText: "Alberto Sejas Knapp\nEinsteinstrasse 59\nDE71229, Leonberg",
    contentLiabilityText: "En tant que prestataire de services, nous sommes responsables de notre propre contenu sur ces pages selon les lois générales conformément au § 7 para. 1 TMG.",
    linksLiabilityText: "Notre offre contient des liens vers des sites Web externes de tiers, sur le contenu desquels nous n'avons aucune influence.",
    copyrightText: "Le contenu et les œuvres créés par les opérateurs du site sur ces pages sont soumis au droit d'auteur allemand.",
    
    // Privacy Policy
    datenschutzTitle: "Politique de Confidentialité",
    dataProtectionHeading: "Protection des données",
    dataCollectionHeading: "Collecte de données",
    cookiesHeading: "Cookies",
    analyticsHeading: "Analyse",
    rightsHeading: "Vos droits",
    contactDataHeading: "Contact protection des données",
    
    dataProtectionText: "Nous prenons très au sérieux la protection de vos données personnelles.",
    dataCollectionText: "Nous collectons et utilisons des données personnelles uniquement dans la mesure nécessaire.",
    cookiesText: "Ce site Web utilise des cookies pour améliorer l'expérience utilisateur.",
    analyticsText: "Nous utilisons des services d'analyse Web pour analyser l'utilisation de notre site Web.",
    rightsText: "Vous avez le droit à l'information, à la correction, à la suppression de vos données personnelles.",
    contactDataText: "Pour les questions concernant la protection des données: [VOTRE EMAIL]",
    
    // Terms of Service
    agbTitle: "Conditions d'Utilisation",
    scopeHeading: "Champ d'application",
    servicesHeading: "Services",
    userObligationsHeading: "Obligations des utilisateurs",
    liabilityHeading: "Responsabilité",
    modificationHeading: "Modifications",
    finalProvisionsHeading: "Dispositions finales",
    
    scopeText: "Ces conditions d'utilisation s'appliquent à tous les services fournis par EVALIA.",
    servicesText: "EVALIA fournit une plateforme de quiz de connaissances alimentée par l'IA.",
    userObligationsText: "Les utilisateurs doivent fournir des informations exactes.",
    liabilityText: "La responsabilité est limitée dans la mesure permise par la loi applicable.",
    modificationText: "Nous nous réservons le droit de modifier ces conditions à tout moment.",
    finalProvisionsText: "Le droit allemand s'applique. Le lieu de juridiction est Leonberg, Allemagne.",
    
    // Contact
    kontaktTitle: "Contact",
    contactFormHeading: "Formulaire de contact",
    nameLabel: "Nom",
    emailLabel: "Email",
    messageLabel: "Message",
    sendButton: "Envoyer le message",
    contactInfoHeading: "Informations de contact",
    addressHeading: "Adresse",
    phoneHeading: "Téléphone",
    emailHeading: "Email"
  },
  
  es: {
    // Navigation
    backToQuiz: "Volver al Quiz",
    
    // Page Names
    impressum: "Aviso Legal",
    datenschutz: "Política de Privacidad",
    agb: "Términos de Servicio",
    kontakt: "Contacto",
    
    // Impressum
    impressumTitle: "Aviso Legal",
    tmgHeading: "Información según § 5 TMG",
    contactHeading: "Contacto",
    taxIdHeading: "ID Fiscal",
    responsibleHeading: "Responsable del contenido según § 55 para. 2 RStV",
    contentLiabilityHeading: "Responsabilidad del contenido",
    linksLiabilityHeading: "Responsabilidad de los enlaces",
    copyrightHeading: "Derechos de autor",
    
    tmgText: "EVAL(IA) - Alberto Sejas Knapp\nEinsteinstrasse 59\nDE71229, Leonberg\nAlemania",
    contactText: "Teléfono: +49 170 967 0950\nEmail: kontakt@evaliaquiz.com",
    taxIdText: "Número de identificación de IVA según § 27 a ley de IVA:\nNº fiscal 97 099 20751",
    responsibleText: "Alberto Sejas Knapp\nEinsteinstrasse 59\nDE71229, Leonberg",
    contentLiabilityText: "Como proveedor de servicios, somos responsables de nuestro propio contenido en estas páginas.",
    linksLiabilityText: "Nuestra oferta contiene enlaces a sitios web externos de terceros.",
    copyrightText: "El contenido y las obras creadas por los operadores del sitio están sujetos a la ley de derechos de autor alemana.",
    
    // Privacy Policy
    datenschutzTitle: "Política de Privacidad",
    dataProtectionHeading: "Protección de datos",
    dataCollectionHeading: "Recopilación de datos",
    cookiesHeading: "Cookies",
    analyticsHeading: "Análisis",
    rightsHeading: "Tus derechos",
    contactDataHeading: "Contacto protección de datos",
    
    dataProtectionText: "Nos tomamos muy en serio la protección de sus datos personales.",
    dataCollectionText: "Recopilamos y utilizamos datos personales solo en la medida necesaria.",
    cookiesText: "Este sitio web utiliza cookies para mejorar la experiencia del usuario.",
    analyticsText: "Utilizamos servicios de análisis web para analizar el uso de nuestro sitio web.",
    rightsText: "Tienes derecho a la información, corrección, eliminación de tus datos personales.",
    contactDataText: "Para preguntas sobre protección de datos: [TU EMAIL]",
    
    // Terms of Service
    agbTitle: "Términos de Servicio",
    scopeHeading: "Alcance de aplicación",
    servicesHeading: "Servicios",
    userObligationsHeading: "Obligaciones del usuario",
    liabilityHeading: "Responsabilidad",
    modificationHeading: "Modificaciones",
    finalProvisionsHeading: "Disposiciones finales",
    
    scopeText: "Estos términos de servicio se aplican a todos los servicios proporcionados por EVALIA.",
    servicesText: "EVALIA proporciona una plataforma de quiz de conocimientos impulsada por IA.",
    userObligationsText: "Los usuarios deben proporcionar información precisa.",
    liabilityText: "La responsabilidad está limitada en la medida permitida por la ley aplicable.",
    modificationText: "Nos reservamos el derecho de modificar estos términos en cualquier momento.",
    finalProvisionsText: "Se aplica la ley alemana. El lugar de jurisdicción es Leonberg, Alemania.",
    
    // Contact
    kontaktTitle: "Contacto",
    contactFormHeading: "Formulario de contacto",
    nameLabel: "Nombre",
    emailLabel: "Email",
    messageLabel: "Mensaje",
    sendButton: "Enviar mensaje",
    contactInfoHeading: "Información de contacto",
    addressHeading: "Dirección",
    phoneHeading: "Teléfono",
    emailHeading: "Email"
  },
  
  it: {
    // Navigation
    backToQuiz: "Torna al Quiz",
    
    // Page Names
    impressum: "Note Legali",
    datenschutz: "Politica sulla Privacy",
    agb: "Termini di Servizio",
    kontakt: "Contatto",
    
    // Impressum
    impressumTitle: "Note Legali",
    tmgHeading: "Informazioni secondo § 5 TMG",
    contactHeading: "Contatto",
    taxIdHeading: "ID IVA",
    responsibleHeading: "Responsabile del contenuto secondo § 55 para. 2 RStV",
    contentLiabilityHeading: "Responsabilità del contenuto",
    linksLiabilityHeading: "Responsabilità dei link",
    copyrightHeading: "Copyright",
    
    tmgText: "EVAL(IA) - Alberto Sejas Knapp\nEinsteinstrasse 59\nDE71229, Leonberg\nGermania",
    contactText: "Telefono: +49 170 967 0950\nEmail: kontakt@evaliaquiz.com",
    taxIdText: "Numero di identificazione IVA secondo § 27 a legge IVA:\nN. fiscale 97 099 20751",
    responsibleText: "Alberto Sejas Knapp\nEinsteinstrasse 59\nDE71229, Leonberg",
    contentLiabilityText: "Come fornitore di servizi, siamo responsabili del nostro contenuto su queste pagine.",
    linksLiabilityText: "La nostra offerta contiene link a siti web esterni di terze parti.",
    copyrightText: "Il contenuto e le opere create dagli operatori del sito sono soggetti alla legge tedesca sul copyright.",
    
    // Privacy Policy
    datenschutzTitle: "Politica sulla Privacy",
    dataProtectionHeading: "Protezione dei dati",
    dataCollectionHeading: "Raccolta dati",
    cookiesHeading: "Cookies",
    analyticsHeading: "Analisi",
    rightsHeading: "I tuoi diritti",
    contactDataHeading: "Contatto protezione dati",
    
    dataProtectionText: "Prendiamo molto sul serio la protezione dei tuoi dati personali.",
    dataCollectionText: "Raccogliamo e utilizziamo dati personali solo nella misura necessaria.",
    cookiesText: "Questo sito web utilizza cookie per migliorare l'esperienza utente.",
    analyticsText: "Utilizziamo servizi di analisi web per analizzare l'uso del nostro sito web.",
    rightsText: "Hai il diritto all'informazione, correzione, cancellazione dei tuoi dati personali.",
    contactDataText: "Per domande sulla protezione dei dati: [LA TUA EMAIL]",
    
    // Terms of Service
    agbTitle: "Termini di Servizio",
    scopeHeading: "Ambito di applicazione",
    servicesHeading: "Servizi",
    userObligationsHeading: "Obblighi dell'utente",
    liabilityHeading: "Responsabilità",
    modificationHeading: "Modifiche",
    finalProvisionsHeading: "Disposizioni finali",
    
    scopeText: "Questi termini di servizio si applicano a tutti i servizi forniti da EVALIA.",
    servicesText: "EVALIA fornisce una piattaforma di quiz della conoscenza alimentata dall'IA.",
    userObligationsText: "Gli utenti devono fornire informazioni accurate.",
    liabilityText: "La responsabilità è limitata nella misura consentita dalla legge applicabile.",
    modificationText: "Ci riserviamo il diritto di modificare questi termini in qualsiasi momento.",
    finalProvisionsText: "Si applica la legge tedesca. Il luogo di giurisdizione è Leonberg, Germania.",
    
    // Contact
    kontaktTitle: "Contatto",
    contactFormHeading: "Modulo di contatto",
    nameLabel: "Nome",
    emailLabel: "Email",
    messageLabel: "Messaggio",
    sendButton: "Invia messaggio",
    contactInfoHeading: "Informazioni di contatto",
    addressHeading: "Indirizzo",
    phoneHeading: "Telefono",
    emailHeading: "Email"
  }
};

// Utility-Funktion für Legal-Übersetzungen
export const getLegalTranslation = (language, key, fallback = '') => {
  const translations = LEGAL_TRANSLATIONS[language] || LEGAL_TRANSLATIONS.en;
  
  // Unterstützung für verschachtelte Keys
  const keys = key.split('.');
  let value = translations;
  
  for (const k of keys) {
    if (value && typeof value === 'object' && k in value) {
      value = value[k];
    } else {
      return fallback || key;
    }
  }
  
  return value || fallback || key;
};

export default getLegalTranslation;